...
- **Boundary placement.** The lateral boundary follows the UWIR 2019 parent-model design (Appendix A, Fig A1-14): every natural pinch-out edge is **no-flow** (the boundary audit showed the perimeter is a thin fringe — thickness p50 6–19 m vs 44 m interior), and **GHBs** sit only on the western/southern truncation faces where the Precipice continues beyond the model frame (`assessment.boundary_mode: "uwir_ghb"`, conductance C = K·b per cell until OGIA's calibrated values arrive; the GHB *head* cancels in twin-differenced drawdown, only conductance matters). A no-flow boundary too close to a pumping bore over-predicts drawdown (conservative); every scenario run reports max boundary drawdown as a QA metric and the UI warns when it is non-trivial.
...
- **Confined layer assumption + linearised unconfined outcrop.** Even in the outcrop, we use `icelltype=0` (confined). True unconfined behaviour would need `icelltype=1` and per-cell saturated thickness updates, which makes the model nonlinear and **breaks superposition**. Instead, the parent regional model's two-part linearisation is reproduced: (1) outcrop cells carry water-table-scale storage — the properties CSV stores a *negative, dimensionless storativity* (the formation-wide outcrop Sy, −1.6293e-2 on 338 of 547 outcrop cells) in the SS column, which `grid.py` decodes as `Ss = |SS|/thickness` so that Ss·b = Sy exactly (UWIR 2025 Fig G.4-30 convention; treating it as a 1/m value would inflate outcrop storage by the cell thickness); (2) rejected recharge is simulated with **drains at the parent model's calibrated surficial-drain cells** (`src/drains.py`): the UWIR 2025 model implements these as RIV reaches with stage == rbot, and `Data/Properties_recharge/riv_cells.csv` (extracted by `scripts/extract_uwir2025.py`) supplies the calibrated stage (drain elevation) and conductance (5,000 m²/d) per cell via `drains.riv_cells_csv` in config.yaml. When that file is absent the fallback is the older device — drain at the minimum DEM elevation within each outcrop cell with an estimated `K·A/b` conductance. The steady-state pre-run uses real DRN cells; the transient runs replace flowing drains with fixed GHBs at the drain elevation ("linearised drains") so the system stays exactly linear and superposition holds. Each run reports `n_drain_reversals` — cells where pumping pulled the head below the drain elevation, i.e. where a real drain would have shut off and the linearisation locally under-predicts drawdown.
