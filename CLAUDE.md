...
| Springs | `Data/Springs/*.shp` | Shared layer; UNION rule at ingest: a spring is kept if it sits within `assessment.spring_outcrop_buffer_m` (default 3 km) of the module's outcrop OR matches `inputs.springs_attr_keep` (aquifer attribution — artesian complexes like Boggomoss discharge through the cover ~8–13 km from outcrop). Springs with no complex are dropped. Points just outside the active domain are sampled at the nearest active cell within `assessment.spring_snap_max_m` (default 3 km). |
