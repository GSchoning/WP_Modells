...
**A proof-of-concept overview** · Version 0.2 (draft) · August 2026 · Document ID: [to be confirmed]
...
### 3.5  Storage conversion and initial heads

Two further parent-model behaviours are reproduced as defaults. First, cell storage converts between elastic storage and specific yield as the head crosses the top of a cell, exactly as in the parent model. Without this conversion, sustained pumping continues to drain elastic storage after a cell has depressurised and over-predicts drawdown; on the Precipice baseline the non-converting treatment inflates mean 100-year drawdown by about a third. Secondly, the initial head field is the parent model's pre-development surface. The initial condition largely cancels in the twin-run formulation (section 3.3), but it sets the discharge available at the spring and baseflow drains: the pre-development surface gives the outcrop drains their observed headroom, where the tool's own single-layer equilibrium, which lacks the regional model's vertical support, would understate that headroom and overstate impacts at the most exposed spring complexes by a factor of two to four. Both behaviours can be switched to their conservative alternatives from the interface for sensitivity comparison, and every result records which configuration produced it.

### 3.6  Impact layers and threshold classification
...
The assessing officer works from a web map. A proposed bore is placed by clicking the map, or several bores for a multi-bore application, or a trade in which extraction is removed at a source bore and added at one or more destinations. Rates are entered in ML/year. Results return in minutes as impact tables, a stacked chart per spring complex separating licensed from S&D impact, drawdown maps, and a recommendation against the threshold. Drawdown at every neighbouring water bore is tabulated alongside the spring complexes. Every scenario also reports two Theis analytical estimates (Theis 1935) beside the numerical results: the proposal alone, and the cumulative impact of all existing bores by superposition of Theis solutions with the department's standard assessment parameters, which is the current-practice method. Divergence between the calibrated model and current practice, arising from heterogeneity, boundaries or storage, is therefore visible to the officer for the case in front of them rather than hidden.

### 4.3  Decision tracking and provenance

Every approve or reject decision is recorded as an event in an append-only register, together with the officer's name, a free-text reason, the timestamp, the full scenario change set, the headline results, hashes of the configuration and input datasets, and the model version. Reversals and rollbacks are themselves recorded events attributed to a named officer; the current status of each decision (active, reversed, rolled back or rejected) is derived by replaying the event history, so nothing is ever rewritten or deleted. Because the full scenario definition travels with each event, any past decision can be re-run and its numbers reproduced exactly from the recorded provenance. The register is stored as durable runtime data, separate from regenerable model outputs, and is intended to support internal review, audit and information-access processes.

The register is also an input to the modelling: extraction approved through the tool feeds directly into subsequent assessments. The well change set of every active approval, including trades with their source-bore reductions, is folded into the approved-take baseline in the background, so the next application is assessed against the cumulative approved state rather than against the static dataset alone. Reversing or rolling back a decision removes its take again; an officer can reverse a single approval, rewind the register to an earlier state, or clear it entirely, and cleared registers are archived rather than deleted. Baselines are cached per register state, so returning to a previously assessed state is immediate. One operational rule follows: when a refreshed water-use dataset begins to include a bore that was approved through the tool, the corresponding decision should be reversed so that its take is not counted twice; automating that retirement is identified as a next step (section 7).

## 5  Verification

The tool carries an automated test suite of 48 tests that runs on every change. Superposition is verified against a directly modelled combined scenario for the linear configuration, both on an idealised model and with the full boundary and storage machinery active, with agreement at solver precision. A dedicated regression test reproduces the spring-discharge behaviour at the outcrop: a bore pumped inside a drain field must first capture the local discharge and then develop a growing cone of depression, and the directly modelled combined scenario is confirmed to never understate impact relative to the added parts. The storage-conversion treatment is verified against a true unconfined solution and against analytical mass balances. Further regression tests pin the vertical-leakage device to the expected analytical behaviour, confirm that the initial-head overlay and the leakage source head cancel in twin-run drawdown, and exercise the approvals register end to end, including trades, reversals, rollbacks and archiving. The numerical engine is verified against the Theis analytical solution for a single bore in a uniform aquifer. End-to-end pipeline tests run on a synthetic case. The engine is MODFLOW 6 (currently version 6.7.0), a United States Geological Survey code in wide international use, invoked unmodified through the FloPy interface (Bakker et al 2016).
...
- Single-layer modules. Each aquifer is modelled alone and inter-aquifer leakage through aquitards is not represented by default. For the Precipice this omission has been quantified with the parent model's own calibrated vertical hydraulic conductivity: the overlying Evergreen sequence is effectively sealed (harmonic-mean vertical conductivity of order 10⁻⁸ m/day), and enabling the derived leakage term shifts the largest spring-complex impact by less than a tenth, so the single-layer treatment is consistent with the parent calibration there. The equivalent check for the Hutton and Gubberamunda is outstanding, and for assessments where vertical connectivity is material the regional model remains the appropriate instrument.
...
- Constant-rate scenarios. Extraction is modelled as constant over the assessment period, the standard conservative licensing assumption; time-varying regimes are not yet supported. In addition, all existing take is applied from the start of the assessment period, where the regional model stages bores on over their drilling history; this is conservative for the cumulative layers, and a like-for-like benchmark against the parent model's staged history is identified in section 7.
...
- Automatic retirement of tool approvals when a refreshed water-use dataset begins to include the approved bore, completing the register-to-baseline integration delivered in this build (section 4.3), which currently relies on the officer reversing the superseded decision.
- A like-for-like benchmark against the parent regional model, using its staged pumping history and its own predicted heads; the extraction tooling for both is in place. This would confirm the pre-development initial-head default and quantify the conservatism of the constant-rate assumption.
...
- Cached baselines and twin runs reduce an assessment to one model run, with the applicant's contribution reported as the marginal impact over existing use; spring and baseflow discharge, storage conversion and the pre-development initial heads behave as in the parent model, and the discharge a proposal captures is itself reported in ML/year.
- Impacts are reported in four layers so that approved, licensed and proposed contributions are visible separately, and threshold exceedances are attributed to the correct cause. Approvals made through the tool join the approved-take baseline of subsequent assessments, so cumulative impact accrues decision by decision within a licensing cycle.
